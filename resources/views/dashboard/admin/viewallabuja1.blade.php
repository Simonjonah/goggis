              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    @foreach ($viewalluyo_students as $viewalluyo_student)
       
                      <tr>
                        <td>{{ $viewalluyo_student->surname }}</td>
                        <td>{{ $viewalluyo_student->middlename }}</td>
                        <td>{{ $viewalluyo_student->fname }}
                        <small>{{ $viewalluyo_student->classname }}</small>
                        <small>{{ $viewalluyo_student->entrylevel }}</small>
                        <small>{{ $viewalluyo_student->academic_session }}</small>
                        <small>{{ $viewalluyo_student->section }}</small>
                        </td>
                        <td><img style="width: 100%; height: 60px;" src="{{ URL::asset("/public/../$viewalluyo_student->images")}}" alt=""></td>
                        <td><a href="{{ url('admin/viewstudents/'.$viewalluyo_student->ref_no) }}"
                            class='btn btn-default'>
                             <i class="far fa-eye"></i>
                         </a></td>
                         <td>@if ($viewalluyo_student->status == null)
                          <span class="badge badge-secondary"> In progress</span>
                         @elseif($viewalluyo_student->status == 'suspend')
                         <span class="badge badge-warning"> Suspended</span>
                         @elseif($viewalluyo_student->status == 'reject')
                         <span class="badge badge-danger"> Rejected</span>
                         @elseif($viewalluyo_student->status == 'approved')
                         <span class="badge badge-info"> Approved</span>
                         @elseif($viewalluyo_student->status == 'admitted')
                      ...
                      <td>{{ $viewalluyo_student->regnumber }}</td>
                      <td>{{ $viewalluyo_student->ref_no }}</td>
                         <td><a href="{{ url('admin/editstudent/'.$viewalluyo_student->ref_no) }}"
                          class='btn btn-info'>
                      ...
                      <th> <a href="{{ url('admin/studentsaddmit/'.$viewalluyo_student->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
