          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Father: {{ $parent->fathername }} {{ $parent->fathersurname }} Mother {{ $parent->mothername }} {{ $parent->mothersurname }}</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                  ...
                  <tbody>
                    ...
                    @foreach ($view_childrens as $view_children)
                      ...
                      <tr>
                        ...
                        <td><a href="{{ url('admin/viewstudents/'.$view_children->ref_no) }}"
                            class='btn btn-default'>
                             <i class="far fa-eye"></i>
                         </a></td>
                         <td>@if ($view_children->status == null)
                          <span class="badge badge-secondary"> In progress</span>
                         @elseif($view_children->status == 'suspend')
                         <span class="badge badge-warning"> Suspended</span>
                         @elseif($view_children->status == 'reject')
                         <span class="badge badge-danger"> Rejected</span>
                         @elseif($view_children->status == 'approved')
                      ...
                      <td>{{ $view_children->ref_no }}</td>

                         <td><a href="{{ url('admin/editstudent/'.$view_children->ref_no) }}"
                          class='btn btn-info'>
                           <i class="far fa-edit"></i>
                       </a></td>  
                       
                        
                       <th><a href="{{ url('admin/rejectstudent/'.$view_children->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-user"></i>
                      </a></th>
                      <th><a href="{{ url('admin/assignedteacher/'.$view_children->centername) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th><th><a href="{{ url('admin/suspendstudent/'.$view_children->ref_no) }}" class="btn btn-sm bg-teal">
                        <i class="fas fa-comments"></i>
                      </a></th>

                      <th> <a href="{{ url('admin/studentsaddmit/'.$view_children->ref_no) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-user"></i> 
                      </a></th>
                      
                     
                      {{-- <th><a href="{{ url('admin/studentit/'.$view_children->ref_no) }}" class="btn btn-info"><i class="fas fa-user"></i> IT</a></th> --}}
                       <td><a href="{{ url('admin/deletestudent/'.$view_children->ref_no) }}"
